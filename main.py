...
def start(message):
    ...
    bot.send_message(message.chat.id, "Привет, это бот выполняющий функции большинства, если не всех продуктов "
                                      "OpenAI. Это полностью не коммерческий бот "
                                      "и поддержать разработчика ты можешь подписавшись на boosty, подробнее командой"
                                      "/pay. Если у вас возникли вопросы по использованию бота, то просмотрите "
...
def pay(message):
    bot.send_message(message.chat.id, "Рад что ты выразил желание помочь, ссылка на boosty: "
                                      "https://boosty.to/gospodin_moderator. При оформлении платной подписки ты "
                                      "получишь приятные плюшки, такие как бета-тестирование новых функций, "
                                      "постоянная связь со мной, доступ к трансляциям, которые я провожу время от "
                                      "времени и доступ к частному телеграмм чату.")
